...
class GraphDataset(data.Dataset):
    ...
    def __init__(self, root, ids, target_patch_size=-1):
        ...
        # self.classdict = {'normal': 0, 'dysplasia': 1}
    ...
    def __getitem__(self, index):
        sample = {}
        # remove \n from train_set.txt or val_set.txt
        info = self.ids[index].replace('\n', '')
        # get the file name and label from train_set.txt or val_set.txt
        file_name, label = info.split('\\t')
        ...
        # convert normal to 0 and dysplasia to 1 and put into sample['label']
        sample['label'] = int(label)
        sample['id'] = file_name

        #feature_path = os.path.join(self.root, file_name, 'features.pt')
        # ../graphs/simclr_files/wsi_name/features.pt
        ...
        # ../graphs/simclr_files/wsi_name/adj_s.pt
