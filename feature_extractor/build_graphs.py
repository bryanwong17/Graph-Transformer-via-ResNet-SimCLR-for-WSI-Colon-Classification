...
def compute_feats(args, bags_list, i_classifier, save_path=None, whole_slide_path=None):
    ...
    for i in range(0, num_bags):
        ...
        print('{} files to be processed'.format(len(csv_file_path)))

        if os.path.isdir(os.path.join(save_path, 'simclr_files_2', file_name)) or len(csv_file_path) < 1:
        ...
        os.makedirs(os.path.join(save_path, 'simclr_files_2', file_name), exist_ok=True)

        txt_file = open(os.path.join(save_path, 'simclr_files_2', file_name, 'c_idx.txt'), "w+")
        save_coords(txt_file, csv_file_path)
        # save node features
        output = torch.stack(feats_list, dim=0).cuda()
        torch.save(output, os.path.join(save_path, 'simclr_files_2', file_name, 'features.pt'))
        # save adjacent matrix
        adj_s = adj_matrix(csv_file_path, output)
        torch.save(adj_s, os.path.join(save_path, 'simclr_files_2', file_name, 'adj_s.pt'))
...
def main():
    ...
    parser.add_argument('--dataset', default="../../dataset/tiles/colon/wsi/*", type=str, help='path to patches')
    ...
    parser.add_argument('--weights', default='runs/Jan27_15-44-39_DESKTOP-35H91QJ/checkpoints/model.pth', type=str, help='path to the pretrained weights')
    parser.add_argument('--output', default='../graphs_test_4', type=str, help='path to the output graph folder')
